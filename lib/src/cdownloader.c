...
int magicnet_chain_downloader_thread_ask_for_blocks(struct magicnet_chain_downloader *downloader)
{
    ...
    MAGICNET_SHARED_MUTEX_OBJECT(struct magicnet_client *) *new_sclient = NULL;
    ...
    magicnet_log("%s asking network for key TO DOWNLOAD BLOCKS FROM\n", __FUNCTION__);
    ...
    new_sclient = magicnet_connect_for_key(downloader->server, key, "chain-downloader");
    if (!new_sclient)
    {
        magicnet_log("%s FAILED\n", __FUNCTION__);
        goto out;
    }
    new_client = magicnet_client_shared_hold(new_sclient);
    if (!new_client)
    {
        magicnet_log("%s FAILED to hold connected client\n", __FUNCTION__);
        goto out;
    }
    ...
    for (int i = 0; i < MAGICNET_MAX_BLOCK_SUPER_DOWNLOAD_REQUEST_BLOCK_COUNT; i++)
    {
        ...
        new_client->flags |= MAGICNET_CLIENT_FLAG_IGNORE_TRANSACTION_AND_BLOCK_VALIDATION;
