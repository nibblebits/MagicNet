...
size_t magicnet_server_clients_count(struct magicnet_server *server);
size_t magicnet_server_outgoing_clients_count(struct magicnet_server *server);

int magicnet_init_client(MAGICNET_SHARED_MUTEX_OBJECT(struct magicnet_client *) * client_shared, struct magicnet_server *server, int connfd, struct sockaddr_in *addr_in);
...
int magicnet_server_poll_client_process(struct magicnet_client *client, struct magicnet_packet *packet);
...
void magicnet_client_hold(struct magicnet_client *client)
{
    ...
    {
        magicnet_bug("%s no shared pointer associated\n", __FUNCTION__);
        return;
    }

    magicnet_shared_mutex_obj_owner_hold(client->our_shared_ptr);
}
...
void magicnet_client_release(struct magicnet_client *client)
{
    ...
    magicnet_shared_mutex_obj_owner_release(client->our_shared_ptr);
}
...
void magicnet_client_lock(struct magicnet_client *client)
{
    ...
    magicnet_shared_mutex_obj_lock(client->our_shared_ptr);
}
...
void magicnet_client_unlock(struct magicnet_client *client)
{
    ...
    magicnet_shared_mutex_obj_unlock(client->our_shared_ptr);
}
...
void magicnet_client_free(void *data_ptr)
{
...
MAGICNET_SHARED_MUTEX_OBJECT(struct magicnet_client *) * magicnet_client_new()
{
    ...
    MAGICNET_SHARED_MUTEX_OBJECT(struct magicnet_client *) *client_shared = NULL;
    client = calloc(1, sizeof(struct magicnet_client));
    if (!client)
    {
        return NULL;
    }

    client_shared = magicnet_shared_mutex_obj_create_hold_as_owner(client, magicnet_client_free, NULL);
    if (!client_shared)
    {
        free(client);
        return NULL;
    ...
    magicnet_init_client(client_shared, NULL, 0, NULL);
...
static MAGICNET_SHARED_MUTEX_OBJECT(struct magicnet_client *) * magicnet_server_client_vec_find_stale_slot(struct vector *sclients_vec)
{
    ...
    while (sclient)
    {
        if (magicnet_shared_mutex_obj_is_stale(sclient))
        {
            return sclient;
        }
...
static void magicnet_server_client_vec_release_all(struct vector *sclients_vec)
{
    if (!sclients_vec)
    {
        return;
    ...
    while (sclient)
    {
        vector_pop_last_peek(sclients_vec);
        magicnet_client_shared_viewer_release(sclient);
        sclient = vector_peek_ptr(sclients_vec);
    }

    vector_free(sclients_vec);
}
...
void magicnet_server_free(struct magicnet_server *server)
{
    ...
    magicnet_server_client_vec_release_all(server->clients);
    magicnet_server_client_vec_release_all(server->outgoing_clients);
    vector_free(server->connected_ips);
...
void magicnet_server_recalculate_my_ip_for_client_vec(struct vector *sclient_vec, struct vector *ip_vec)
{
    vector_set_peek_pointer(sclient_vec, 0);
    MAGICNET_SHARED_MUTEX_OBJECT(struct magicnet_client *) *sclient = vector_peek_ptr(sclient_vec);
    while (sclient)
    {
        struct magicnet_client *client = magicnet_client_shared_hold(sclient);
        if (client && magicnet_connected(client))
        {
            magicnet_ip_count_vec_add_or_increment(ip_vec, client->my_ip_address_to_client);
        }
        ...
        sclient = vector_peek_ptr(sclient_vec);
    }
}
void magicnet_server_recalculate_my_ip(struct magicnet_server *server)
{
    ...
    // Adjust the ip vector based on the input and output vectors
    // we shall discover my true IP decentralized.
    magicnet_server_recalculate_my_ip_for_client_vec(server->clients, ip_vec);
    magicnet_server_recalculate_my_ip_for_client_vec(server->outgoing_clients, ip_vec);

    // Why do we do this, we do it as people might lie about our IP to us.
    // we could just ping a random server somewhere that will tell us our IP
    // but then its a centralized point of failure
    // but this needs improving too.
...
bool magicnet_client_shared_in_use(MAGICNET_SHARED_MUTEX_OBJECT(struct magicnet_client *) * client_shared)
{
    if (!client_shared)
    {
        return false;
    ...
    return !magicnet_shared_mutex_obj_is_stale(client_shared);
}
...
size_t magicnet_server_clients_count(struct magicnet_server *server)
{
    return vector_count(server->clients);
}
 ...
 * i want a slot recycling type of thing, like the previous design but better because
 * of the vector aspect.
 */
MAGICNET_SHARED_MUTEX_OBJECT(struct magicnet_client *) * magicnet_find_free_client(struct magicnet_server *server)
{
    MAGICNET_SHARED_MUTEX_OBJECT(struct magicnet_client *) *sclient = NULL;
    struct magicnet_client *client = NULL;
    sclient = magicnet_server_client_vec_find_stale_slot(server->clients);
    if (!sclient && vector_count(server->clients) >= MAGICNET_MAX_INCOMING_CONNECTIONS)
    {
        return NULL;
    }
    ...
    if (sclient)
    {
        if (magicnet_shared_mutex_obj_fill_hold(sclient, client, magicnet_client_free) < 0)
        {
            free(client);
            return NULL;
        }
    ...
    sclient = magicnet_shared_mutex_obj_create_hold_as_owner(client, magicnet_client_free, NULL);
    if (!sclient)
    {
        free(client);
        return NULL;
    ...
    magicnet_server_client_vec_add(server->clients, sclient);
    return sclient;
}

MAGICNET_SHARED_MUTEX_OBJECT(struct magicnet_client *) * magicnet_find_free_outgoing_client(struct magicnet_server *server)
{
    MAGICNET_SHARED_MUTEX_OBJECT(struct magicnet_client *) *sclient = NULL;
    struct magicnet_client *client = NULL;
    sclient = magicnet_server_client_vec_find_stale_slot(server->outgoing_clients);
    if (!sclient && vector_count(server->outgoing_clients) >= MAGICNET_MAX_OUTGOING_CONNECTIONS)
    {
        return NULL;
    }
    ...
    magicnet_server_client_vec_add(server->outgoing_clients, sclient);
    return sclient;
}
...
int magicnet_init_client(MAGICNET_SHARED_MUTEX_OBJECT(struct magicnet_client *) * client_shared, struct magicnet_server *server, int connfd, struct sockaddr_in *addr_in)
{
    ...
    // be super careful with this, our own data must be deletedbefore the mutex
    // at all times for this to be safe.
    client->our_shared_ptr = client_shared;
...
MAGICNET_SHARED_MUTEX_OBJECT(struct magicnet_client *) * magicnet_tcp_network_connect_for_ip_for_server(struct magicnet_server *server, const char *ip_address, int port, const char *program_name, int signal_id, int flags)
{
    ...
    magicnet_server_lock(server);
    mclient_shared = magicnet_find_free_outgoing_client(server);
    if (!mclient_shared)
    {
        magicnet_server_unlock(server);
        close(sockfd);
    ...
    magicnet_init_client(mclient_shared, server, sockfd, &servaddr);
    struct magicnet_client *mclient = magicnet_shared_mutex_obj_owner_hold(mclient_shared);
...
{
    return amount > MAGICNET_magicnet_request_input_data_MAXIMUM_SIZE;
}
...
int magicnet_client_read_magicnet_request_response_output_data(struct magicnet_client *client, struct magicnet_request_response_output_data **output_data_out, struct buffer *store_in_buf)
{
    int res = 0;
    struct magicnet_request_response_output_data *req_res_output_data = NULL;
    char *output_data_ptr = NULL;
    size_t output_len = 0;
    *output_data_out = NULL;
    res = magicnet_client_read_unknown_bytes(client, &output_data_ptr, &output_len, MAGICNET_magicnet_request_response_output_data_MAXIMUM_SIZE, store_in_buf);
    if (res < 0)
    {
        goto out;
    }
...
int magicnet_client_read_magicnet_request_input_data(struct magicnet_client *client, struct magicnet_request_input_data **input_data_out, struct buffer *store_in_buf)
{
    int res = 0;
    struct magicnet_request_input_data *req_res_input_data = NULL;
    char *input_data_ptr = NULL;
    size_t input_data_len = 0;
    *input_data_out = NULL;
    res = magicnet_client_read_unknown_bytes(client, &input_data_ptr, &input_data_len, MAGICNET_magicnet_request_input_data_MAXIMUM_SIZE, store_in_buf);
    if (res < 0)
    {
        goto out;
    }
...
int magicnet_client_read_request_packet(struct magicnet_client *client, struct magicnet_packet *packet_out)
{
    int res = 0;
    ...
    res = magicnet_client_read_magicnet_request_input_data(client, &input_data, packet_out->not_sent.tmp_buf);
    if (res < 0)
    {
        goto out;
    }
    ...
    magicnet_signed_data(packet_out)->payload.request.signal_id = signal_id;
...
int magicnet_client_read_request_response_packet(struct magicnet_client *client, struct magicnet_packet *packet_out)
{
    int res = 0;
    struct magicnet_request_input_data *input_data = NULL;
    ...
    res = magicnet_client_read_magicnet_request_response_output_data(client, &output_data, packet_out->not_sent.tmp_buf);
    if (res < 0)
    {
        goto out;
    }
...
int magicnet_client_write_magicnet_request_response_output_data(struct magicnet_client *client, struct magicnet_request_response_output_data *output_data, struct buffer *store_in_buf)
{
    int res = 0;
...
int magicnet_client_write_magicnet_request_input_data(struct magicnet_client *client, struct magicnet_request_input_data *input_data, struct buffer *store_in_buf)
{
    int res = 0;
...
int magicnet_client_write_packet_request(struct magicnet_client *client, struct magicnet_packet *packet_in)
{
    int res = 0;

    // Lets check theirs an input data to send
    if (!magicnet_signed_data(packet_in)->payload.request.input_data)
    {
    ...
    res = magicnet_write_int(client, magicnet_signed_data(packet_in)->payload.request.type, packet_in->not_sent.tmp_buf);
    if (res < 0)
    {
        goto out;
    }
    ...
    // Write the flags
    res = magicnet_write_int(client, magicnet_signed_data(packet_in)->payload.request_response.flags, packet_in->not_sent.tmp_buf);
    if (res < 0)
    {
        goto out;
    }
    ...
    // Write the signal id
    res = magicnet_write_int(client, magicnet_signed_data(packet_in)->payload.request.signal_id, packet_in->not_sent.tmp_buf);
    if (res < 0)
    {
        goto out;
    }
    ...
    res = magicnet_client_write_magicnet_request_input_data(client, magicnet_signed_data(packet_in)->payload.request.input_data, packet_in->not_sent.tmp_buf);
    if (res < 0)
    {
        goto out;
    }
...
int magicnet_client_write_packet_request_response(struct magicnet_client *client, struct magicnet_packet *packet_in)
{
    int res = 0;
    // First things first lets validate that theirs an actual request input and output to send
    if (!magicnet_signed_data(packet_in)->payload.request_response.input_data || !magicnet_signed_data(packet_in)->payload.request_response.output_data)
    {
    ...
    // Write the request type
    res = magicnet_write_int(client, magicnet_signed_data(packet_in)->payload.request_response.type, packet_in->not_sent.tmp_buf);
    if (res < 0)
    {
        goto out;
    }
    ...
    // Write input data
    res = magicnet_client_write_magicnet_request_input_data(client, magicnet_signed_data(packet_in)->payload.request_response.input_data, packet_in->not_sent.tmp_buf);
    if (res < 0)
    {
        goto out;
    }
    ...
    res = magicnet_client_write_magicnet_request_response_output_data(client, magicnet_signed_data(packet_in)->payload.request_response.output_data, packet_in->not_sent.tmp_buf);
    if (res < 0)
    {
        goto out;
    }
...
MAGICNET_SHARED_MUTEX_OBJECT(struct magicnet_client *) * magicnet_connect_again_incoming(struct magicnet_client *client, const char *program_name)
{
    ...
    res = magicnet_signal_wait_timed(signal, 30, (void **)&sclient_out);
    if (res < 0)
    {
...
MAGICNET_SHARED_MUTEX_OBJECT(struct magicnet_client *) * magicnet_connect_again(MAGICNET_SHARED_MUTEX_OBJECT(struct magicnet_client *) * sclient, const char *program_name)
{
    ...
    {
        cloned_sclient = magicnet_connect_again_outgoing(client, program_name);
    }
    else if (conn_type == MAGICNET_CONNECTION_TYPE_INCOMING)
    {
        cloned_sclient = magicnet_connect_again_incoming(client, program_name);
    }
    else
    {
...
void magicnet_copy_packet_request(struct magicnet_packet *packet_out, struct magicnet_packet *packet_in)
{
    magicnet_signed_data(packet_out)->payload.request.input_data = magicnet_reqres_input_data_clone(magicnet_signed_data(packet_in)->payload.request.input_data);
}

void magicnet_copy_packet_request_response(struct magicnet_packet *packet_out, struct magicnet_packet *packet_in)
{
    magicnet_signed_data(packet_out)->payload.request_response.input_data = magicnet_reqres_input_data_clone(magicnet_signed_data(packet_in)->payload.request_response.input_data);
    magicnet_signed_data(packet_out)->payload.request_response.output_data = magicnet_reqres_output_data_clone(magicnet_signed_data(packet_in)->payload.request_response.output_data);
}
...
void magicnet_copy_packet(struct magicnet_packet *packet_out, struct magicnet_packet *packet_in)
{
    ...
    case MAGICNET_PACKET_TYPE_REQUEST:
        magicnet_copy_packet_request(packet_out, packet_in);
        break;

    case MAGICNET_PACKET_TYPE_REQUEST_RESPONSE:
        magicnet_copy_packet_request_response(packet_out, packet_in);
        break;
    }
...
{
    int res = 0;
    // round robin type of setup here, looks decent.
...
void magicnet_server_relay_packets_to_clients_vec(struct vector *sclients_vec, struct magicnet_packet *packet)
{
    ...
    {
        struct magicnet_client *client = magicnet_client_shared_hold(sclient);
        if (client && magicnet_client_in_use(client))
        {
...
int magicnet_server_add_packet_to_relay(struct magicnet_server *server, struct magicnet_packet *packet)
{
    ...
    magicnet_server_relay_packets_to_clients_vec(server->outgoing_clients, packet);
...
//     if (magicnet_signed_data(packet)->payload.sync.flags & MAGICNET_TRANSMIT_FLAG_EXPECT_A_PACKET)
//     {
//         res = magicnet_server_poll_client_process(client, magicnet_signed_data(packet)->payload.sync.packet);
//         if (res < 0)
//         {
//             goto out;
//         }
...
int magicnet_client_process_request(struct magicnet_client *client, struct magicnet_packet *packet)
{
    int res = 0;
    int id = magicnet_signed_data(packet)->payload.request.id;
    int flags = magicnet_signed_data(packet)->payload.request.flags;
    int signal_id = magicnet_signed_data(packet)->payload.request.signal_id;
    int request_type = magicnet_signed_data(packet)->payload.request.type;
    struct magicnet_request_input_data *input_data = magicnet_signed_data(packet)->payload.request.input_data;

    struct magicnet_reqres_request *req = magicnet_reqres_request_new(id, request_type, signal_id, input_data);
    if (!req)
    {
        goto out;
    }
    magicnet_reqres_lock(client->reqres);
    res = magicnet_reqres_request_push(client->reqres, req);
    if (res < 0)
    {
...
out:
    ...
    if (req)
    {
        magicnet_reqres_request_free(req);
        req = NULL;
    }
    return res;
}
...
int magicnet_client_process_request_response(struct magicnet_client *client, struct magicnet_packet *packet)
{
    ...
    int request_id = magicnet_signed_data(packet)->payload.request_response.id;
    int request_type = magicnet_signed_data(packet)->payload.request_response.type;
    int request_signal_id = magicnet_signed_data(packet)->payload.request_response.signal_id;
    ...
    // we received from our server peer force into the client local connection
    ...
    // Alright inject it straight into our local client lets do it
    magicnet_reqres_lock(client->reqres);
    res = magicnet_reqres_response_push(client->reqres, reqres_res);
    if (res < 0)
    {
        magicnet_reqres_unlock(client->reqres);
        goto out;
    ...
    // Now the local client also has it in their stack
    // they can pop when they are ready
 ...
 * \deprecated This is not used see the replacement function magicnet_server_poll_process
 */
int magicnet_client_process_packet(struct magicnet_client *client, struct magicnet_packet *packet)
{
    // WARNING THIS DOES NOT APPEAR TO BE USED!
...
{
    int res = 0;
    vector_set_peek_pointer(server->clients, 0);
    MAGICNET_SHARED_MUTEX_OBJECT(struct magicnet_client *) *sclient = vector_peek_ptr(server->clients);
    while (sclient)
    {
        struct magicnet_client *client = magicnet_client_shared_hold(sclient);
        if (client && magicnet_connected(client) && magicnet_client_localhost(client) &&
        ...
        sclient = vector_peek_ptr(server->clients);
    }
...
int magicnet_server_push_client_info_to_peer_exchange_vector(struct vector *sclients_vec, struct vector *peer_vec_out)
{
    int res = 0;
    vector_set_peek_pointer(sclients_vec, 0);
    MAGICNET_SHARED_MUTEX_OBJECT(struct magicnet_client *) *sclient = vector_peek_ptr(sclients_vec);
    while (sclient)
    {
        ...
        // Client is connected proceed.
        struct magicnet_connection_exchange_peer_data data = {0};
        data.public_key = client->peer_info.key;
        data.sin_addr = client->client_info.sin_addr;
        vector_push(peer_vec_out, &data);
        magicnet_client_shared_release(sclient);
        sclient = vector_peek_ptr(sclients_vec);
    }
    return res;
...
int magicnet_client_thread_poll(struct magicnet_nthread_action *action)
{
    ...
    MAGICNET_SHARED_MUTEX_OBJECT(struct magicnet_client *) *client_shared = action->private;
    ...
    if (res < 0)
    {
        // already held from the creation of this thread
...
int magicnet_client_reqres_begin_polling(struct magicnet_client *client)
{
    // Pushes the reqres poll thread action so it runs on its own thread
    // in the thread pool.
    magicnet_reqres_begin_polling(client->reqres);
    return 0;
}
...
{
    int res = 0;
    res = magicnet_client_push(client_shared);
    if (res < 0)
    {
        goto out;
    }

    // Server clients need the reqres action running so they can handle them
    // local clients will not use this system in this way.
    // polling wont be required.
    struct magicnet_client *client = magicnet_client_shared_hold(client_shared);
    res = magicnet_client_reqres_begin_polling(client);
...
{
    int res = 0;
    struct magicnet_nthread_action *thread_action = NULL;
    ...
    // we shall see.
    // we are an owner because the cl ient must exist when a thread is running.
    ...
    thread_action = magicnet_threads_action_new(magicnet_client_thread_poll, client_shared, NULL);
    if (!thread_action)
    {
        magicnet_shared_mutex_obj_owner_release(client_shared);
        return -1;
...
static int magicnet_server_begin_polling_shared_client(MAGICNET_SHARED_MUTEX_OBJECT(struct magicnet_client *) * sclient)
{
    ...
    res = magicnet_client_push(sclient);
    if (res < 0)
    {
        client = magicnet_client_shared_hold(sclient);
        if (client)
        {
            magicnet_client_close(client);
            magicnet_client_shared_release(sclient);
    ...
    // Release the owner ref returned by the connect/accept path.
    magicnet_client_shared_release(sclient);
    return res;
}
...
int magicnet_server_process_make_new_connection_packet(struct magicnet_client *client, struct magicnet_packet *packet)
{
    ...
    MAGICNET_SHARED_MUTEX_OBJECT(struct magicnet_client *) *new_client = magicnet_tcp_network_connect_for_ip_for_server(client->server, ip, MAGICNET_SERVER_PORT, program_name, magicnet_signed_data(packet)->payload.new_connection.entry_id, 0);
    if (new_client)
    {
        // Push the new client to the thread pool for regular polling.
        res = magicnet_server_begin_polling_shared_client(new_client);
    }
...
int magicnet_server_poll_client_process(struct magicnet_client *client, struct magicnet_packet *packet)
{
    ...
    // Handle only localhost client packets here.
    if (magicnet_client_localhost(client))
    {
        switch (magicnet_signed_data(packet)->type)
        {

            // User defined can also be sent from localhost as well as public
        case MAGICNET_PACKET_TYPE_USER_DEFINED:
    ...
    {
        // Here we handle the public server packets, more security focused.
        switch (magicnet_signed_data(packet)->type)
        {
        case MAGICNET_PACKET_TYPE_USER_DEFINED:
            res = magicnet_server_poll_process_user_defined_packet(client, packet);
...
//     res = magicnet_server_poll_client_process(client, packet);
//     if (res < 0)
//     {
//         goto out;
//     }
...
bool magicnet_server_is_outgoing_connection_connected(struct magicnet_server *server, const char *ip_address)
{
    ...
    vector_set_peek_pointer(server->outgoing_clients, 0);
    MAGICNET_SHARED_MUTEX_OBJECT(struct magicnet_client *) *sclient = vector_peek_ptr(server->outgoing_clients);
    while (sclient)
    {
        struct magicnet_client *client = magicnet_client_shared_hold(sclient);
        if (client && magicnet_client_in_use(client))
        ...
        sclient = vector_peek_ptr(server->outgoing_clients);
    }
    return false;
}
...
bool magicnet_server_is_ip_connected(struct magicnet_server *server, const char *ip_address)
{
    ...
    while (ip_addr)
    {
        ...
        ip_addr = vector_peek_ptr(server->connected_ips);
    }
...
void magicnet_server_attempt_new_connections(struct magicnet_server *server)
{
    ...
    MAGICNET_SHARED_MUTEX_OBJECT(struct magicnet_client *) *client = magicnet_tcp_network_connect_for_ip_for_server(server, ip, MAGICNET_SERVER_PORT, MAGICNET_LISTEN_ALL_PROGRAM, 0, 0);
    if (client)
    {
        magicnet_server_begin_polling_shared_client(client);
    }
